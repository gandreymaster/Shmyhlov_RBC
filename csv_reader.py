...
def input_tuples():
    L = []
    print("Введіть зв'язки у ланцюжку у форматі 'x y', де x і y - цілі числа, або натисніть Enter для завершення введення")
    while True:
        try:
            inp = input().strip()
            if not inp:
            ...
            print("Невірний формат. Спробуйте знову.")
    return L

def input_delivery_stage():
    print("Введіть delivery stages у форматі 'x y z ...', де x, y, z - цілі числа:")
    try:
        D_st = tuple(map(int, input().split()))
    except ValueError:
        print("Невірний формат. Спробуйте знову.")
        D_st = input_delivery_stage()
    return D_st
...
filepath = input("Введіть шлях до файлу з даними:")

with open(filepath, mode='r', encoding='utf-8-sig') as file:
    reader = csv.DictReader(file)
    for row in reader:
        stage.append(int(row['stage']))
        c_i.append(float(row['c_i']))
        t_i.append(int(row['t_i']))
        C_i.append(float(row['C_i']))
        M_i.append(int(row['M_i']))
        sigma_i.append(float(row['sigma_i']))

I_st = len(stage)

# const
z = map(int, input("Введіть z_i"))
h = map(int, input("Введіть holding cost"))
